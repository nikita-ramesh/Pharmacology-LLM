...
def load_and_split_training_data(file_path, split_ratio=0.5, random_seed=42):
    try:
        ...
        df = df[['ID', 'Natural Language Query', 'Notes for student', 'SQL', '2nd SQL', 'Training/test set', 'Minimum output columns']]
...
def run_test_set():
    ...
    training_data, test_data = load_and_split_training_data('Training/all_queries_categorised_train.csv')
    ...
    with open("split_results.txt", "w", encoding="utf-8") as result_file:
    ...
    conn.close()
    print("Results have been written to 'split_results.txt'.")
